...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    // Tìm các item được gán cho staff theo trạng thái
    @Query(value = "SELECT oi FROM OrderItem oi JOIN FETCH oi.order WHERE oi.assignedStaff = :staff AND oi.stringingStatus = :status", countQuery = "SELECT COUNT(oi) FROM OrderItem oi WHERE oi.assignedStaff = :staff AND oi.stringingStatus = :status")
    ...
    long countByAssignedStaffAndStringingStatusIn(@org.springframework.data.repository.query.Param("staff") Staff staff,
